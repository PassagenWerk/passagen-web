...
论文库以只读 MCP tools 和 resources 提供给 LibreChat、桌面 MCP Host 和自定义 Agent。

```text
Passagen Web / CLI  -- read and write --> data directory
Passagen MCP Server -- read only ------> data directory
...
搜索、代码执行、任务规划和写作工具的复杂工作流，但不会修改论文库、启动 processing 或调用
LLM 生成新 artifact。
...
- 让 Web 保持论文库的 writer，而 MCP 保持应用层只读。
...
或离线完整性维护前，应同时停止 Web 和 MCP；正常运行时保持单个 Web writer，MCP 只执行读取。
...
services:
  passagen-mcp:
    image: ${PASSAGEN_MCP_IMAGE:-docker.io/sycstudio/passagen-mcp-server:0.1.0}
